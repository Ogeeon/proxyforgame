Look at @changelog.sql file. The `ru` row holds the current message; it is the only source of truth.

The file is generated by `make changelog-release` from the `> **RU:**` quote of the newest release in `CHANGELOG.md`, which writes the Russian text into all twelve rows. Do not edit it by hand and do not edit the `ru` row — if the Russian text is wrong, fix the quote in `CHANGELOG.md` and re-run the release.

Rewrite the `description` of every other language row for the same `id` so it is a translation of the current Russian text. Ignore whatever those rows contain right now — they are leftovers from a previous changelog entry, so do not treat a mismatch as a conflict, do not ask about it, and do not report it as a problem. Never leave a non-`ru` row describing anything other than the current Russian message.

Rules:
- Keep the exact set of languages already present in the file — do not add or drop any.
- Keep the one-line `insert into change_descriptions (id, lang, description) values (...) on duplicate key update description = values(description);` format and the existing row order. Only the `description` literal changes.
- Escape single quotes as `''` (SQL literal escaping).
- Use the official OGame terminology of each language for game entities (buildings, technologies, ships, Life Forms).
- Keep every translation under 1024 characters — that is the width of `change_descriptions.description`, and MySQL truncates anything longer without an error. Translations run longer than the Russian source, French by roughly a quarter, so a source near the limit needs tightening rather than a literal translation.
