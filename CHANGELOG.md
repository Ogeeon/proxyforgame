...
- Flight: the Mechan General Enhancement now deepens the general's deuterium discount by the full percentage entered, not a fifth of it. Speed and cargo were moved onto that scale when the enhancement was first added; the fuel column kept the old one, so a general with life forms was quoted far too much deuterium - a player with a 62,88% enhancement saw 6.530 against the game's 1.708 on a recycler run. The discount is also capped at 100%, which the enhancement can now reach on its own. Reported by a player with the API 2 export and the in-game figures to compare against; the remaining gap on that report is still open.
