...
| `www/ogame/calc/js/trade.js` | `trade` only — it never went through the module split |
| `www/ogame/calc/css/<name>_bs.css` | |
| `unit-tests/<name>-core.test.js` | present for 6 of the 10 |
...
Six of the ten have a unit test: `expeditions`, `flight`, `graviton`, `moon`, `production`,
`terraformer`. For `costs`, `lfcosts`, `queue` and `trade` there is no `*-core.test.js` —
